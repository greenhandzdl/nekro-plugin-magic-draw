# 高级绘图插件 (Magic Draw)

> NekroAgent 的高级图像生成插件，为 AI 提供复杂图像处理能力。

## 🎨 插件简介

高级绘图插件是内置绘图功能的增强补充，通过精心设计的提示词工程和自动化后处理流程，实现普通绘图难以完成的特殊效果。

## ✨ 主要功能

### 1. GIF 动画生成

生成流畅的循环 GIF 动画，支持像素风格、卡通风格等多种画风。

**特点：**

- 自动生成 16 帧连续动画
- 智能提取和处理透明背景
- 支持自定义画面风格和帧率
- 动画可无缝循环播放

**使用示例：**

```
请帮我生成一个像素风格的 GIF 动画，内容是一只橘猫在奔跑
```

### 2. 透明背景 PNG 生成

自动生成带透明背景的 PNG 图片，无需手动抠图。

**特点：**

- AI 自动选择对比色背景
- 智能识别并移除背景色
- 边缘处理自然流畅
- 适合制作贴纸、图标等素材

**使用示例：**

```
请帮我生成一张透明背景的卡通猫咪图片
```

### 3. 角色概念设定图生成

基于参考图生成专业的角色深度概念分解图（Character Sheet），适用于游戏美术、动漫设计、角色设定等场景。

**特点：**

- 全景式角色深度概念分解
- 中心立绘 + 周边拆解元素环绕
- 服装分层、内着细节、表情集、材质特写
- 生活化物品展示（包袋内容、美妆护理、个人物件）
- 专业注释说明，4K 高清输出

**使用示例：**

```
基于这个角色图片，生成一张完整的角色设定图，展示她的服装细节和生活物品
```

## 🚀 如何使用

### 安装插件

1. 在 NekroAgent 管理界面进入"插件管理"
2. 搜索"高级绘图"或"magic-draw"
3. 点击安装并启用插件

### 配置说明

在插件设置中，您需要配置以下选项：

- **基础绘图模型组**：用于快速、简单的辅助绘图任务
- **高级绘图模型组**：用于复杂、精细的绘图任务（推荐使用 Gemini 3 等高性能模型）
- **使用流式 API**：启用后可避免长时间处理导致超时，强烈推荐保持开启
- **请求超时时间**：图像生成需要较长时间，默认 300 秒，根据模型性能可适当调整
- **GIF 帧边缘过滤像素数**：处理 GIF 时过滤掉边缘的像素数，用于移除 AI 可能生成的分割栅格，默认 4 像素
- **GIF 默认帧率**：GIF 动画的默认帧率（每秒帧数），默认 8 FPS。AI 可以根据动画类型自行设定合适的帧率，未指定时使用此默认值
  - 快速动作（如奔跑、挥动）：12-15 FPS
  - 中速动作（如走路、呼吸）：8-10 FPS
  - 慢速动作（如飘动、闪烁）：6-8 FPS
- **调试模式**：开启后会在聊天中显示生成的中间图片，方便查看处理过程

### 与 AI 对话使用

插件安装后，您可以直接通过对话让 AI 使用高级绘图功能：

- "生成一个像素风格的跑步小人 GIF 动画"
- "画一只透明背景的卡通猫咪"
- "制作一个循环播放的火焰动画"
- "基于这个角色，生成一张完整的设定图，展示她的服装和生活细节"

AI 会自动识别需求并调用相应的高级绘图功能。

## 🔧 技术原理

### GIF 动画生成

**非透明模式（默认）：**
1. 通过特殊提示词指导 AI 生成 4×4 网格的序列图（16 格）
2. 自动切割为 16 帧动画
3. 裁剪边缘像素（移除可能的分割栅格）
4. 直接输出不透明的循环 GIF 动画

**透明背景模式（可选）：**
1. 提示 AI 生成全部 16 帧动画，要求使用纯色背景
2. 自动切割并裁剪边缘像素（移除分割栅格）
3. 从所有帧边缘提取最常见颜色作为背景色
4. 将背景色替换为透明像素，输出透明 GIF

### 透明 PNG 生成

1. 提示 AI 使用与主体对比强烈的纯色背景绘制
2. 分析图片边缘像素，识别背景色
3. 将背景色替换为透明像素（支持容差）
4. 输出透明背景的 PNG 图片

## 📋 系统要求

- NekroAgent 版本 >= 2.0.0
- Python >= 3.10
- 需要配置支持聊天图像生成的 AI 模型

## 🎯 推荐模型

为获得最佳效果，推荐使用以下模型：

- **Gemini 3 Pro Image Preview**：速度快、效果最好、支持高复杂指令引导
- **Gemini 2.5 Flash Image Preview**：速度快、效果好、支持大多数指令
- **DALL-E 3 / Sora Image**：效果出色，价格较高，速度较慢

## ⚠️ 注意事项

1. 高级绘图功能比普通绘图耗时更长，请耐心等待
2. 生成效果受 AI 模型能力影响，建议使用推荐模型

## 🔗 相关链接

- [NekroAgent 官方文档](https://doc.nekro.ai/)
- [插件开发指南](https://doc.nekro.ai/docs/04_plugin_dev/intro.html)
- [问题反馈](https://github.com/KroMiose/nekro-plugin-magic-draw/issues)
- [社区交流群](https://qm.qq.com/q/hJlRwD17Ae)：636925153

## 📄 开源协议

MIT License

---

**享受创作的乐趣！** 🎨✨
